...
Unfortunately the water shut off during this experiment, so results were lost. The experiment was not able to be re-conducted prior to the submission of this report.

## Conclusions
Based on the above data, the HRS team is unable to conclude whether the recycle line will actually help the longevity of the floc blanket. Results thus far have not been promising, but different approaches and tests are ongoing. The team believes that adjusting the entrance of the tap itself may affect the results (buildup of the floc blanket around the tap entrance) and that incorporating a peristaltic pump to reverse water flow in the favored direction to increase floc blanket longevity could also make the recycle line effective.

Experiment 2 demonstrated that performing the test with the recycle line closed for the initial portion of the experiment yielded the same results as Experiment 1 despite hopes that doing so would allow for the floc blanket to establish itself more effectively.

Experiment 3 involved a new approach; the design of the tap at the top of the recycle line was taken into consideration and a new one was installed such that the tap was flush with the inside of the tube settler. This experiment's results are currently not solidified as the experiment is still in progress. However, the team hopes that adjusting the tap will prevent the buildup of the floc blanket in the upper part of the tube settler near the entrance of the tap.

Despite these experiments, both the cause of rapid floc blanket decay and how the recycle line may help to mitigate this issue are not known. Ongoing work and future directions are discussed in the future work section below.


## Future Work
Future directions for Flow Recycle include incorporating a peristaltic pump along the recycle line to force water in the correct direction that will allow for the floc blanket to be sustained longer. Other options include fabricating a tap that is sanded down at the round portion that enters the sedimentation tank. Doing so would hopefully minimize any additional tap surface area that may be contributing to the buildup of flocs during experiments. The subteam might also test at various upflow velocities (potentially lower), considering all tests thus far this semester have been conducted at 3 mm/s. It is possible that the instability of the floc blanket could be related to the upflow velocity if varying the recycle line still does no benefit to the sedimentation tank system in future experiments.
...
Figure 12: The ON state of the HRS: Flow Recycle ProCoDA method file.
...
Figure XX: This is the overall order of the Set Points for the HRS method file.
...
Figure 14: These are the output settings for the PID state. This process runs in the background and only needs to be set up at the beginning of the semester.
...
Figure 15: This shows the influent turbidity Set Point and the selected sensors required in order to establish this relationship.
